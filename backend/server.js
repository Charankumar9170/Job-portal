...
app.post("/post-job", (req, res) => {
  ...
  const sql = `
    INSERT INTO Jobs 
      (employee_id, company, title, location, work_mode, skills, salary, qualification, responsibilities, description, job_id, date) 
    VALUES (
      ?,                                              -- employee_id
      (SELECT organisation FROM Employer WHERE id = ?), -- company from Employer
      ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
    )
  `;
  ...
  const values = [
    id,           // employee_id
    id,           // used in subquery for company
    title,
    location,
    work_mode,
    skills,
...
app.get("/fetch-jobs", (req, res) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {
    ...
    const fetchJobsQuery = `
      SELECT Sno, employee_id, company ,title, location, work_mode, skills, salary, qualification, responsibilities, description,job_id,date
      FROM Jobs 
      WHERE employee_id = ?
    `;
...
//------------------DTELETE JOB-----------
app.post("/employee/delete-job", (req, res) => {
  const { job_id } = req.body; // extract job_id field
  ...
  db.query(deleteQuery, [job_id], (err, result) => {
    if (err) {
...
//------------------UPDATE JOB-------------------
app.post("/employer/job-update", (req, res) => {
  const {
    job_id,
    title,
    location,
